...
def parseHashPoint(point, numBuckets):
    ...
    tuplas = hashFunction(numBuckets, lista).items()
    
    vector = SparseVector (numBuckets,tuplas)
    return LabeledPoint (labelFeatures[0], vector)
...
def computeSparsity(data, d, n):
    ...
    """
    return data.map (lambda lp:len (lp.features.indices)).sum () / float (n * d)
